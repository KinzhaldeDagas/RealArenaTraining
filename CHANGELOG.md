...
- Decoded and used Oblivion's lower-level player skill-progress routine at `0x00668B30` for raw progress application.
- Confirmed `Player_ModExperience` at `0x00668C30` depends on a selected skill-use index before forwarding progress, so RealArenaTraining avoids inventing a use-index semantic.
- Decoded static melee hit and impact paths used to resolve player hits against arena practice statics.
- Decoded arrow collision and static hit paths used to award `TargetHay01` Marksman progress.
- Confirmed skill progress array access through `ActorValue_GetGroupOffsetFromAV(2, actorValue)`.

### Scope

- RealArenaTraining does not change normal actor or NPC combat skill behavior.
- RealArenaTraining does not grant progress for unrelated statics, actors, creatures, or arbitrary collision objects.
- The custom path is limited to the supported decoded practice statics listed above.
